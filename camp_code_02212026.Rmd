...
server <- function(input, output, session) {
  ...
  # Memoised wrappers (disk cache shared across sessions on the host)
  getClimate_cached <- memoise::memoise(getClimate, cache = PRISM_CACHE)
  getNorm_cached    <- memoise::memoise(getNorm,    cache = PRISM_CACHE)

  # ---- species choices (use FIA or fixed list) ----
  # If you want FIA-derived species, replace this with your FIA species extraction.
  species_master <- sort(c(
    "Pinus ponderosa","Pinus contorta","Pinus jeffreyi","Pinus lambertiana","Pinus monticola",
  ...
  observeEvent(TRUE, {
    updateCheckboxGroupInput(
      session, "species_selected",
      choices = species_master,
      selected = species_master
    )
  }, once = TRUE)

  observeEvent(input$selectAll, {
    updateCheckboxGroupInput(session, "species_selected", selected = species_master)
  })
  ...
  # ---- run PRISM + getSeeds when year/species change ----
  # Debounce prevents hammering PRISM while the slider is being dragged
  year_db <- shiny::debounce(reactive(input$year), millis = 600)
  sp_db   <- shiny::debounce(reactive(input$species_selected), millis = 600)

seed_points <- eventReactive(input$runClimate, {
  req(input$year, input$species_selected)
...
output$map <- renderLeaflet({
  ...
  validate(need(!is.na(value_col), "No value column found to map."))

  pal <- leaflet::colorNumeric(c("red", "blue"), seeds[[value_col]])

  leaflet::leaflet(seeds) %>%
    ...
    leaflet::addLegend("bottomright", pal = pal, values = seeds[[value_col]],
                       title = "Predicted", opacity = 1)
})

  # ---- remap whenever seed_points changes ----
  observe({
    seeds <- seed_points()
    req(seeds)
    validate(need(nrow(seeds) > 0, "No seed predictions returned for this selection."))

    # Pick a value column to color by (change this to your actual output column)
    value_col <- dplyr::case_when(
      "logF_current" %in% names(seeds) ~ "logF_current",
      "fecundity"    %in% names(seeds) ~ "fecundity",
      "Value"        %in% names(seeds) ~ "Value",
    ...
    validate(need(!is.na(value_col), "Could not find a value column to map (e.g., logF_current/fecundity/Value)."))

    pal <- colorNumeric(c("red", "blue"), seeds[[value_col]])
    ...
    leafletProxy("map", data = seeds) %>%
      ...
      addLegend("bottomright", pal = pal, values = seeds[[value_col]], title = "Predicted", opacity = 1)
  })

  output$statusText <- renderText({
    if (isTruthy(input$year) && isTruthy(input$species_selected)) {
      "Ready. Updating map when inputs change."
    } else {
      "Select a year and species."
    }
  })
}
```
