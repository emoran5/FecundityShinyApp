...
getClimate <- function(species, year) {
  ...
  tree_sf <- sf::st_as_sf(trees, coords = c("UTMx", "UTMy")) # turn data frame into a spatial object
  
  tcrs <- "+proj=longlat +datum=WGS84"
  sf::st_crs(tree_sf) <- tcrs
  tree_vec <- terra::vect(tree_sf) # convert tree file to spatial vector
...
##### 
  ...
  params <- filter(all_params, Taxa %in% Taxa)
...
getNorm <- function(Species, year) {
  ...
  tree_sf <- st::st_as_sf(trees, coords = c("UTMx", "UTMy")) # turn data frame into a spatial object
...
server <- function(input, output, session) {
  ...
  treeNames <- reactive({
    ...
    merge(PLOT_TREE, sp_names) %>%
      ...
      dplyr::filter(Species %in% Species)  # <-- filter to your defined list
  })
  ...
  seed_points <- eventReactive(input$runClimate, {
    ...
    # Critical: functions defined outside server() won't see tn unless we put it where they look.
    assign("treeNames", tn, envir = .GlobalEnv)
    assign("treeNames", tn, envir = environment(getClimate))
    assign("treeNames", tn, envir = environment(getNorm))
    ...
    # You said coordinates are already corrected: X/Y are lon/lat
